...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        $fileModel = File::find()->where(['md5'=>$md5])->one();
        if($fileModel){
            if($fileModel['path'] && file_exists(Yii::getAlias('@images').'/'.$fileModel['path'])){
                $this->stateInfo = "SUCCESS";
                $this->fullName = '/images/'.$fileModel['path'];return;
            }else{
                $fileModel->delete();
            }
        }
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            $fileModel = new File();
            $fileModel->path = substr($this->fullName,7);
            $fileModel->md5 = $md5;
            $fileModel->sha1 = $sha1;
            $fileModel->save();
